...
class Turn:
    ...
    def __init__(self, turn_id, transcript, turn_label, belief_state, system_acts,
                 system_transcript, num=None):
        self.id = turn_id
    ...
    def to_dict(self):
        return {'turn_id': self.id, 'transcript': self.transcript, 'turn_label': self.turn_label,
                ...
                'system_transcript': self.system_transcript, 'num': self.num}
    ...
    def annotate_raw(cls, raw):
        ...
        return cls(
            ...
            turn_label=[[fix.get(s.strip(), s.strip()), fix.get(v.strip(), v.strip())] for s, v in
                        raw['turn_label']],
...
class Dataset:
    ...
    def iter_turns(self, limit=None):
        if limit:
            self.dialogues = self.dialogues[:limit]
    ...
    def evaluate_preds(self, preds):
        ...
        for d in self.dialogues:
            ...
            for t in d.turns:
                ...
                i += 1
                gold, pred = get_turn_pred_gold(gold_request, gold_inform, gold_recovered,
                                                pred_request, pred_inform,
                                                pred_recovered)
                # trans = str(t.transcript) + "\n" + str(t.system_transcript)
                trans = str(t.transcript) + "\n" + str(t.belief_state)
                tee.add_row([trans, gold, pred])
                # print(gold_request)
                # print(gold_inform)
...
class Ontology:
    ...
    def __add__(self, another):
        ...
        new_values = {s: sorted(list(set(self.values.get(s, []) + another.values.get(s, [])))) for s
                      in new_slots}
    ...
    def numericalize_(self, vocab):
        ...
        for s, vs in self.values.items():
            self.num[s] = [
                vocab.word2index(annotate('{} = {}'.format(s, v)) + ['<eos>'], train=True) for v in
                vs]
