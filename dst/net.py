...
class GLADEncoder(nn.Module):
    ...
    def __init__(self, din, dhid, slots, dropout=None):
        ...
        for s in slots:
            setattr(self, '{}_rnn'.format(s),
                    nn.LSTM(din, dhid, bidirectional=True, batch_first=True,
                            dropout=self.dropout.get('rnn', 0.)))
            setattr(self, '{}_selfattn'.format(s),
    ...
    def forward(self, x, x_len, slot, default_dropout=0.2):
        ...
        h = F.dropout(local_h,
                      ...
                      self.training) * beta + \
            F.dropout(global_h, self.dropout.get('global', default_dropout), self.training) * (
                        1 - beta)

        c = F.dropout(local_selfattn(h, x_len),
                      self.dropout.get('local', default_dropout),
                      self.training) * beta + \
            F.dropout(self.global_selfattn(h, x_len),
                      self.dropout.get('global', default_dropout), self.training) * (1 - beta)
        return h, c
...
class GladModel(nn.Module):
    ...
    def __init__(self, args, ontology, vocab):
        ...
        self.emb_fixed = FixedEmbedding(len(vocab), args.get('demb'),
                                        dropout=args['dropout'].get('emb', 0.2))

        self.utt_encoder = GLADEncoder(args['demb'], args['dhid'], self.ontology.slots,
                                       dropout=args['dropout'])
        self.act_encoder = GLADEncoder(args['demb'], args['dhid'], self.ontology.slots,
                                       dropout=args['dropout'])
        self.ont_encoder = GLADEncoder(args['demb'], args['dhid'], self.ontology.slots,
                                       dropout=args['dropout'])
    ...
    def forward(self, batch):
        ...
        for s in self.ontology.slots:
            ...
            q_utts = []
            for c_val in C_vals:
                q_utt, _ = attend(H_utt, c_val.unsqueeze(0).expand(len(batch), *c_val.size()),
                                  lens=utterance_len)
                q_utts.append(q_utt)
    ...
    def run_train(self, train, dev, args):
        ...
        for epoch in range(args['epoch']):
            ...
            summary.update(
                {'eval_train_{}'.format(k): v for k, v in self.run_eval(train, args).items()})
            summary.update(
                {'eval_dev_{}'.format(k): v for k, v in self.run_eval(dev, args).items()})
            ...
            if best.get(stop_key, 0) <= summary[stop_key]:
                ...
                self.save(
                    ...
                    identifier='epoch={epoch},iter={iteration},train_{key}={train},dev_{key}={dev}'.format(
                        epoch=epoch, iteration=iteration, train=best_train, dev=best_dev,
                        key=args['stop'],
                    )
                )
                self.prune_saves()
    ...
    def extract_predictions(self, scores, threshold=0.5):
        ...
        for s in self.ontology.slots:
            for i, p in enumerate(scores[s]):
                triggered = [(s, v, p_v) for v, p_v in zip(self.ontology.values[s], p) if
                             p_v > threshold]
                if s == 'request':
